...
def show():
    ...
    """
    This is a funny floating image shower, a small, auto-rotating image gallery
    ---
    you open the program
    and select the image folder
    then as you click on the photo, 
     it switches to the next
    --
    You can select the number of seconds to show each image = 2 seconds
    """
    ...
    refresh_font = sg.user_settings_get_entry('-refresh font-', 'Courier 6')
    ...
    def make_window(location):
        ...
        window = sg.Window('Photo Frame', layout, location=location, no_titlebar=True, grab_anywhere=True,
                           margins=(0, 0),
    ...
    def main():
        ...
        if image_folder is None and single_image is None:
            while True:
                images = None
                image_folder = sg.popup_get_folder('Choose location of your images', location=window.current_location(),
                                                   keep_on_top=True)
        ...
        while True:  # Event Loop
            ...
            if event == 'Edit Me':
                ...
                layout = [[sg.T('Enter number of seconds each image should be displayed')],
                          ...
                          [sg.Ok(), sg.Cancel()]]
                event, values = sg.Window('Display duration', layout, location=window.current_location(),
                                          keep_on_top=True,
                ...
                layout = [[sg.T('Enter size should be shown at in pixels (width, height)')],
                          ...
                          [sg.Ok(), sg.Cancel()]]
                event, values = sg.Window('Image Dimensions', layout, location=window.current_location(),
                                          keep_on_top=True,
                                          no_titlebar=True).read(close=True)
                if event == 'Ok':
                    try:
